...
def draw_evolution(time,location,pop):
    ...
    fig,axs = plt.subplots(2,1,dpi=100,sharex=True, gridspec_kw={'hspace': 0.5})
    ...
    axs[0].set_title("Population 1")
    
    
    axs[1].plot(iteration,res_1[:,0],label='Left',alpha=0.2)
    axs[1].plot(iteration,res_1[:,1],label='Still',alpha=0.5)
    axs[1].plot(iteration,res_1[:,2],label='Right',alpha=0.8)
    axs[0].set_title("Population 2")
