...
class stoc_reg_DiscretizedGraphonExactOMDSolverFinite(Solver):
    """
            ...
            for t in range(mfg.time_steps).__reversed__():
                ...
                for x in range(mfg.agent_observation_space[1].n):
                    ...
                    for u in range(mfg.agent_action_space.n):
                        temp_samples = np.random.choice(range(mfg.agent_observation_space[1].n), self.K, 1,
                                             p=mfg.transition_probs(t, x, u, mu))
                        transit_est = []
                        for temp_S in range(mfg.agent_observation_space[1].n):
                            temp_count = np.sum(temp_samples==temp_S)/self.K
